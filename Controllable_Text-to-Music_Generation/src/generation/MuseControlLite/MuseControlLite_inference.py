...
if __name__ == "__main__":
    ...
    with open("../audio-flamingo/caption.json", "r", encoding="utf-8") as f:
            infos = json.load(f)

    tars_root = "../../../home/fundwotsai/Deep_MIR_hw2/target_music_list_60s"
    tars = os.listdir(tars_root)
    tars_path = [os.path.join(tars_root, tar_path) for tar_path in tars]

    text = []
    for tar in tars_path:
        text.append(infos[tar])

    # print(text)

    config["audio_files"] = tars_path
    config["text"] = text
    
    gen_infos = {}
    for i in range(len(tars_path)):
        temp = {}
        temp["text"] = text[i]
        temp["gen"] = os.path.join("generated_audio/output", f"test_{i}.wav")
        gen_infos[tars_path[i]] = temp

    with open("gen_infos.json", "w", encoding="utf-8") as f:
        json.dump(gen_infos, f, ensure_ascii=False, indent=4)
