...
from MusDr.musdr.side_utils import (
#   get_event_seq, 
  get_bars_crop, 
  get_pitch_histogram, 
...
def parse_opt():
    ...
    parser.add_argument('--dict_path', type=str,
                        help='the dictionary path', default='src/basic_event_dictionary.pkl')
    parser.add_argument('--output_file_path', type=str,
                        help='the output file path.', default='results_task1')
    parser.add_argument('--real_data', type=int,
                        default=1)
    # chord
...
def prepare_data(midi_path, chord):
    # extract events
    events = extract_events(midi_path, chord)
    # event to word
    words = []
    for event in events:
...
if __name__ == "__main__":
  ...
  if opt.real_data == True:
    test_pieces = glob('Pop1K7/midi_analyzed/**/*.mid', recursive=True)
  else:
  ...
  # print avg
  avg_h1 = np.mean(result_dict['H1'])
  avg_h4 = np.mean(result_dict['H4'])
  avg_gs = np.mean(result_dict['GS'])

  print(f"Average H1: {avg_h1:.4f}")
  print(f"Average H4: {avg_h4:.4f}")
  print(f"Average GS: {avg_gs:.4f}")
