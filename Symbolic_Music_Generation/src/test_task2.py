...
def parse_opt():
    ...
    parser.add_argument('--prompt_dir', type=str, default='prompt_song')
...
def test(opt, n_target_bar = 32, prompt = False):
    # check path folder
    try:
        os.makedirs(f'{opt.output_dir}', exist_ok=True)
    except:
        pass
    ...
    with torch.no_grad():
        ...
        utils.write_midi(
            ...
            output_path=os.path.join(opt.output_dir, f"{prompt[:-4]}_config{opt.config}.mid"),
            prompt_path=None)
        
def main():
    opt = parse_opt()

    prompt_mid_list = os.listdir(opt.prompt_dir)
    # generate midi
    for prompt in prompt_mid_list:
        test(opt, prompt=prompt)
    print("task2 finish generate midi")

    # convert to wav
    midi_list = os.listdir(opt.output_dir)
    print(f"generated midi_list: {midi_list}")
    for midi in midi_list:
        print(os.path.join(opt.output_dir, f"{midi}"))
